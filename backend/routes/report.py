...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
If the user message below contains a "REPORT PLAN" block, a separate planning pass already looked at
this request and this source data and proposed a section list + per-section format — follow it as your
structural starting point (see the instructions inside that block for how much you may still adapt it).
Everything in this REPORT STRUCTURE section still governs HOW you execute each planned section — the
plan sets the skeleton, these rules still set the content quality bar. If no REPORT PLAN block is
present (planning was skipped or failed), plan the structure yourself exactly as described below.
...
- NEVER write a standalone line of repeated characters as a decorative section break — no "---",
  "***", "___", "///", "===", or similar. Markdown's own "---" horizontal rule is fine ONLY between
  two paragraphs where a visual break is genuinely wanted; anything else in that family (especially
  "///") has no meaning to the renderer and shows up as literal characters in the finished report.
  Use a new heading or a blank line to separate sections instead.
...
- TABLE/CHART ATTRIBUTION: every table or chart whose numbers come from a specific named publication, dataset, or filing (e.g. "RBI Monetary Policy Report", "Screener.in", "Q1 FY26 earnings release", a named news outlet) MUST carry that name in a "source" field on the chart-spec object itself (e.g. {"type":"table", ..., "source":"RBI Monetary Policy Report"} or {"type":"bar", ..., "source":"Screener.in"}), in addition to any inline prose mention. This renders as a visible credit line under the published chart/table — leaving it out means the chart displays as if it were Growth Gradual's own data, which is a misattribution, not merely a missing one. Only omit "source" for a chart/table you genuinely derived yourself from multiple inputs with no single citable origin (e.g. a ratio you computed across several rows) — in that case it correctly falls back to being labelled as Growth Gradual's own analysis. If you write a markdown table directly in the report body instead of a JSON chart-spec, add a line immediately after the table in the form "*Source: <name>*" — this is parsed automatically into the same attribution field.
...
def _validate_image_selections(parsed_images: list, candidates: list[dict]) -> tuple[list[dict], list[bool]]:
    ...
    for entry in parsed_images or []:
        ...
        final.append({
            "url": candidate["url"],
            "caption": caption or candidate["description"] or "",
            # Every image pulled from a web candidate must carry its source
            # domain through to render — an image with no visible credit is
            # exactly the "used without attaching source" problem.
            "source": candidate.get("domain") or "",
        })
        mask.append(True)
    return final, mask
...
# Matches a "*Source: X*" / "_Source: X_" attribution line the model is
# instructed to place directly under a hand-written markdown table (see the
# TABLE/CHART ATTRIBUTION prompt rule). Captured into the chart-spec's
# "source" field and consumed here so it isn't ALSO left behind as loose
# italic prose right under the rendered table.
_MD_TABLE_SOURCE_RE = re.compile(r"^[ \t]*[*_]{1,2}\s*Source:\s*(.+?)\s*[*_]{1,2}[ \t]*$", re.IGNORECASE)
    ...
    while i < n:
        ...
        if header_m and i + 1 < n and _MD_TABLE_SEP_RE.match(lines[i + 1]):
            ...
            if len(header_cells) >= 2 and len(rows) >= 2 and not is_metadata_table and not is_duplicate_of_chart:
                ...
                # Look for a "*Source: X*" line immediately after the table
                # (skipping at most one blank line) and fold it into the
                # chart-spec's own "source" field instead of leaving it as a
                # separate line of prose — same attribution field the JSON
                # chart path uses, so both routes into a table render the
                # credit line identically.
                table_source = ""
                k = j
                if k < n and not lines[k].strip():
                    k += 1
                if k < n:
                    src_m = _MD_TABLE_SOURCE_RE.match(lines[k])
                    if src_m:
                        table_source = src_m.group(1).strip()
                        j = k + 1
    ...
    for i, entry in enumerate(raw):
        ...
        if 1 <= idx <= len(candidates):
            ...
            resolved[i] = {
                ...
                # Source-backed image — always carry the originating domain
                # through so the renderer can show a visible credit.
    ...
    if ai_jobs:
        ...
        for (orig_idx, _prompt, caption), result in zip(ai_jobs, results):
            ...
            resolved[orig_idx] = {
                ...
                # Not pulled from any website — label it as such rather than
                # leaving the credit line blank, which would look identical
                # to an unattributed real photo.
...
# ── Explicit structure-planning step ───────────────────────────────────────
# Previously the ONLY thing deciding a report's section list and per-section
# presentation (table vs chart vs bullets vs prose) was the writer LLM call
# itself, guided by the REPORT STRUCTURE / FORMAT RATIO prose inside
# SYSTEM_PROMPT — i.e. structure and prose generation happened in the same
# pass, from the same call, under the same token budget pressure. That works
# but is entirely prompt-driven: there is no point where the model looks at
# what data actually showed up in the sources and commits to a plan before
# it starts writing.
#
# This adds a small, separate planning call: given the question and a LIGHT
# preview of the source material (titles/snippets/short excerpts — NOT the
# full enriched text the writer gets), a cheap, fast model proposes a section
# list with a purpose and a presentation format per section, plus an overall
# depth call. That plan is then handed to the writer as explicit guidance
# (see _format_plan_for_prompt) alongside — never instead of — every existing
# rule in SYSTEM_PROMPT (data integrity, chart rules, no-repetition, JSON
...
def _validate_report_plan(plan) -> bool:
    ...
    if not isinstance(sections, list) or not (1 <= len(sections) <= 14):
        return False
    allowed_formats = {"prose", "table", "chart", "bullets", "mixed"}
    seen_headings = set()
    for sec in sections:
        ...
        if norm in seen_headings:
            return False  # duplicate section — planner malfunctioned, don't trust it
        seen_headings.add(norm)
        if sec.get("format") not in allowed_formats:
            return False
...
def _build_plan_source_preview(
    ...
    deliberately much smaller than the full src_text the writer gets. The
    planner needs to see what KINDS of data exist (rankings, time series,
    tables, thematic prose) to choose sections/formats sensibly; it does not
    need every fetched character, and keeping this small keeps the planning
    call fast and cheap and clear of Groq's prompt-size budget."""
    parts: list[str] = []
    if file_section.strip():
        parts.append("UPLOADED FILE / STRUCTURED SOURCE DATA (preview):\n" + file_section.strip()[:2500])
    if okf_context.strip() and okf_context.strip() not in file_section:
        parts.append("STRUCTURED DOCUMENT CONTEXT (preview):\n" + okf_context.strip()[:1500])
    if enriched_sources:
        src_lines = []
        for s in enriched_sources[:max_sources]:
            title = (s.get("title") or "").strip()
            excerpt = (s.get("fullContent") or s.get("snippet") or "").strip()[:per_source_chars]
            if not title and not excerpt:
                continue
            src_lines.append(f"- {title}: {excerpt}")
        if src_lines:
            parts.append(f"WEB SOURCE PREVIEW ({len(src_lines)} of {len(enriched_sources)} sources):\n" + "\n".join(src_lines))
    if no_web_sources:
        parts.append(
            "NO SOURCES: no web sources or file data were found for this request — plan a shorter, "
            "prose/knowledge-driven structure with no chart/table sections invented from nothing."
        )
    if not parts:
        parts.append("No source preview available.")
    return "\n\n".join(parts)
...
async def _plan_report_structure(
    ...
    data, BEFORE the writer generates any report text. Returns None on any
    failure so callers can fall back to the writer's existing implicit
    planning (REPORT STRUCTURE rules in SYSTEM_PROMPT) unchanged."""
    source_preview = _build_plan_source_preview(
        question, enriched_sources, file_section, okf_context, no_web_sources,
    )
    depth_cue = ""
    if wants_more_data_viz:
    ...
    try:
        plan = await _call_llm_json(PLANNING_SYSTEM_PROMPT, planning_user_prompt)
    except Exception as e:
        log.warning("Report: planning call raised, falling back to implicit planning: %s", e)
        return None

    if not _validate_report_plan(plan):
        log.warning("Report: planner returned invalid/malformed plan (%r) — falling back to implicit planning",
                     plan if not isinstance(plan, dict) else list(plan.keys()))
        return None
...
def _format_plan_for_prompt(plan: dict) -> str:
    """Render a validated plan into the block injected into the writer's
    user_prompt. Framed as explicit structural guidance the writer should
    follow, not a rigid template it must reproduce verbatim — the writer
    keeps every existing SYSTEM_PROMPT rule (data integrity, chart rules,
    no-repetition, JSON shape, etc.) on top of this."""
    lines = [
        "REPORT PLAN (from a separate planning pass over THIS request and THIS source data — "
    ...
    for i, sec in enumerate(plan["sections"], 1):
        ...
        lines.append(
            f"{i}. \"{sec['heading']}\" — {sec.get('purpose', '').strip()} "
            f"[format: {sec['format']}" + (f" — {reason}" if reason else "") + "]"
        )
...
async def generate_report(request: Request):
    ...
    # Decide the section list and per-section presentation format from the
    # request + the actual source material, in a separate call, before the
    # writer generates any prose. See PLANNING_SYSTEM_PROMPT / _plan_report_structure
    # above. Failure here is non-fatal — falls back to the writer's existing
    # implicit planning (REPORT STRUCTURE rules in SYSTEM_PROMPT), unchanged.
    report_plan = await _plan_report_structure(
        question=question,
